...
contract NFTRentalMarket is Recoverable, Pausable, PayoutWallet, UsingUniversalForwarding, INFTDelegation {
    ...
    uint256 public immutable PERIOD_IN_SECONDS;

    uint256 public immutable OPERATOR_SHARE;
    ...
    ) UsingUniversalForwarding(forwarderRegistry, universalForwarder) PayoutWallet(msg.sender, operator) Pausable(false) {
    ...
    function rent(uint256 tokenId, uint256 duration) external {
        ...
        if (lease.lessee != address(0)) {
            ...
            require(block.timestamp > lease.startDate + previousDuration * PERIOD_IN_SECONDS, "NFTRental: already rented");
    ...
    function updateListing(
        ...
        if (lease.lessee != address(0)) {
            // there was a previous lease
            uint256 elapsedPeriods = (block.timestamp - lease.startDate) / PERIOD_IN_SECONDS + 1;
    ...
    function _prepareTokenWithdrawal(uint256 tokenId, address payable sender) internal {
        ...
        uint256 duration = lease.duration;
        if (lease.lessee != address(0)) {
            require(block.timestamp > lease.startDate + duration * PERIOD_IN_SECONDS, "NFTRental: still rented");
        ...
        )
    {
        ...
        if (owner != address(0)) {
            ...
            if (lessee != address(0) && lease.startDate + lease.duration * PERIOD_IN_SECONDS < block.timestamp) {
    ...
    function recoverERC721s(
        ...
        for (uint256 i = 0; i != length; ++i) {
            ...
            if (recoveredContract == NFT_CONTRACT) {
    ...
    function _computePayoutSharing(uint256 amount) internal view returns (uint256 ownerAmount, uint256 operatorAmount) {
        operatorAmount = (amount * OPERATOR_SHARE) / 100000;

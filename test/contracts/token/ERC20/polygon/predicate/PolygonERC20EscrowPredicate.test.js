...
describe('PolygonERC20EscrowPredicate', function () {
  const fixtureLoader = createFixtureLoader(accounts, web3.eth.currentProvider);

  const fixture = async function () {
    this.predicate = await artifacts.require('PolygonERC20EscrowPredicate').new(rootChainManager, {from: deployer});

    const forwarderRegistry = await artifacts.require('ForwarderRegistry').new();
    this.token = await artifacts.require('ERC20Mock').new([holder], [One], forwarderRegistry.address, ZeroAddress, {from: deployer});
  };
